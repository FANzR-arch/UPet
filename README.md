...
UPet 是播放器，不负责造宠物。目前造新宠物最靠谱的方式是 **Codex**：
在 Codex 里生成的宠物存放在 `C:\Users\<你>\.codex\pets\<宠物名>\spritesheet.webp`，
用右键菜单「导入精灵图…」选中它，宠物就搬到桌面上了——之后 Codex 开不开都无所谓。

也可以使用任何符合格式的精灵图：**8 列 × 11 行**的透明 WebP/PNG，行顺序为
